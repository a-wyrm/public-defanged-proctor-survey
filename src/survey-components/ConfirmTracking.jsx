...
export function TrackingConfirmationButton(Survey) {
  const componentName = "confirm_tracking";
  const activateTracking = {
    ...
    htmlTemplate:
      "<div>" +
      "<button type='button' id='confirm_tracking' class='bg-gw-primary-blue rounded px-8 py-4 mb-5 shadow-lg text-2xl font-extrabold text-white focus:outline-none focus:ring focus:ring-red-500'> Confirm Live Tracking Permissions </button>" +
      "&nbsp;&nbsp;" +
      "</div>",

      // TODO: Add back GWUSECID verification
    afterRender: () => {
      const buttonCustom = document.getElementById("confirm_tracking");

      function setVar () {
        localStorage.setItem("turnOnFaceTracking", "true");
      }
        ...
        if (document.getElementById("GWSECExtensionID")) {
          if (Cookies.get('treatment') == "AI"){
            try {
               ...
               chrome.runtime.sendMessage(extensionID, {greeting: "treatment", type: "AI"});
